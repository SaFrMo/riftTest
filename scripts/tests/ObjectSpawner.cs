...
public class ObjectSpawner : MonoBehaviour
{
    ...
    public GameObject GenerateObject ()
    {
        ...
        Color _color = availableColors[UnityEngine.Random.Range(0, 3)];
        // generate the object's pattern
        Texture2D _texture = availablePatterns[UnityEngine.Random.Range(0, 3)];
        // and lastly, generate the object's shape
        PrimitiveType _shape = availableShapes[UnityEngine.Random.Range(0, 3)];
        
        // apply shape, texture, and color
        GameObject result = GameObject.CreatePrimitive (_shape);
        Renderer _render = result.AddComponent<Renderer>();
		_render.material.mainTexture = _texture;
        _render.material.color = _color;

		// put at a random position
		float posX = (AdjustParameters.XOnlyOnscreen ? UnityEngine.Random.Range (0, 100) / 100 :
